...
自己写脚本测试![img](https://img-blog.csdnimg.cn/9c4aa32c13e44532b794a27edc2717c7.png)
...
前端性能测试 http://testingpai.com/article/1598954323769
...
总结： ![image-20230831203056017](/Users/z.m/Library/Application Support/typora-user-images/image-20230831203056017.png)![img]()
...
+ 回顾
  ...
  + 分布式环境部署
    ...
    + 扩展：
      ...
      + 3、有多台机器作为助攻机器时，活跃线程数，多条； 响应时间曲线，是不做任何处理，原样展示；tps曲线，原样展示。![image-20230831214626539](/Users/z.m/Library/Application Support/typora-user-images/image-20230831214626539.png)
      + 4、CLI命令行模式 -r参数
      + 有一个特别注意：网络
...
+ 性能测试CICD
  ...
  + Jenkins + jmeter + ant 解决方案
    + 为什么选择它？
      ...
      + ant： 代码的编译、测试、执行----- 做什么事情，选择最顺手的，就是最好
        + 最重要的文件： build.xml
        + maven： pom.xml
        + graddle
    + 它有什么作用？
...
+ 回顾
  ...
  + build.xml
    + project:  有且仅有一个
      + default=all   默认情况下，会执行任务名称为all的任务，all的任务，它包含了test、report、mail  也就是说，默认情况下，会执行测试入围、生成报告任务、发送邮件任务。![img](https://img-blog.csdnimg.cn/ad53ebdd94474c27bacde2f07b18958f.png)
...
模拟出来客户的bug，能用生产的实际数据来操作吗？

不可以，要对生产数据向量化，实现数据偏移，要有一个一定的偏移规则，进行脱敏



数据流。对数据流监控，要对服务器监控，到那个微服务消耗比较高，TPS比较低，就知道了水桶效应里面的短板，提高了短板，整体的业务流指标就上来了。



性能测试要在独立的性能测试环境进行性能测试环境

头部公司做全链路性能测试采用生产环境来做，

1. 创建影子库/表，日志隔离，创建独立的库表，发起请求带上性能测试的标签，对数据进行分流
2. 或者使用在headers里面添加性能测试的标签，识别到性能测试标签后，在网关层实现数据分流

中小微企业在独立的性能测试环境进行性能测试环境

![img](https://img-blog.csdnimg.cn/3e25e73839894f1ba46c8f2fae1d35f7.png)
...
+ 服务器  ：  在一些**硬件** + 操作系统 + 应用程序  ===输出 应用能力输出
  ...
  + 我们要写书：
    ...
    + 再精细分工
+ 服务器：![img](https://img-blog.csdnimg.cn/45de2fe2417942d7992850232aa3a5f9.png)
  + 服务器的演变，其实就是我们不断追求高性能，高性价比的一个结果。
  + 服务器一：
    + 单一服务器、 应用服务和数据库服务分离、 应用服务与文件服务器分离+ 数据库服务器
    + linux
    + 应用程序：程序本身  + 架构![image-20230902210429320](/Users/z.m/Library/Application Support/typora-user-images/image-20230902210429320.png)![img](https://img-blog.csdnimg.cn/6521b48b02174d8982ef0bde581c5f80.png)
  + 服务器二：
  + 服务器三：
  + 服务器四：![img](https://img-blog.csdnimg.cn/0356901caf424a1e8f3b0f2811f60fa5.png)

+ Linux
  + centos、ubuntu、suse、redhat、debain、deepin![img](https://img-blog.csdnimg.cn/9c0b0ff491cb48a7b12b7f8fe37f6d2e.png)
  + debian家族： ubuntu
    + 安装命令： apt  apt-get
    + .deb 
  + fedora家族： centos
...
`tree `
 ...
 + linux 的根路径![img](https://img-blog.csdnimg.cn/daa4f2b983aa45a3a427c0463cae2805.png)

 + ![img](https://img-blog.csdnimg.cn/9be25954bf124e8ab9796848d5702f1b.png)
...
+ ls -lth
  + d 目录  l 链接  - 文件
    + 每3个一组： r读  w写  x执![img](https://img-blog.csdnimg.cn/c8b32b0cbce24cc0aecf45eff954e2bd.png)
+ 安装： 
  + fedora家族
    + centos
    + 可执行文件   rpm
...
+ linux性能分析命令![img](https://img-blog.csdnimg.cn/31b35341ab3e426c8670bcc9ea0772d7.png)
  + 第1个 top\ htop\ atop
    + top 查看系统**进程**的资源使用情况， 也可以查看线程
    + top - 21:28:23(当前系统时间) up  1:30(当前系统启用多长时间),  3 users,  load average: 0.00, 0.01, 0.05
      ...
      +  load average: 0.00, 0.01, 0.05  系统瓶颈负载值
        ...
        + 系统负载值，不等于cpu使用率值。因为系统的负载值，它主要由两部分组成： cpu的使用率 + io使用率
          ...
          + io：换入 换出
          + cpu使用率高： us sy ni hi si![img](https://img-blog.csdnimg.cn/505ae12709d54a52a4af39debbe6c06c.png)
        + us：用户态使用cpu的时间占比
      ...
      + 数字 1  可以看到cpu的数量， 核数
    + Tasks: 112 total进程数,   1 running在运行, 111 sleeping休眠,   0 stopped,   0 zombie
    ...
    +  Mem :  1881936 total,   808532 free,   468140 used,   605264 buff/cache
      ...
      + buff/cache: 缓存
        + buffer 缓冲区是由磁盘虚拟出来的，加快读取速度的空间
        + cache 缓存
      + swap: 交互分区,内存数据转移的,计算速度慢
  + vmstat![img](https://img-blog.csdnimg.cn/b6f9d0b881734ae4992f2a5eb20f49d8.png)
  + pidstat
  + iostat
  + netstat
  + sar....
...
+ 任何一个程序启动，都会在内存中占用：虚拟内存核物理内存![img](https://img-blog.csdnimg.cn/684cd9f70ccd40228de52c7aefd5df34.png)
...
+ %MEM     进程使用men率
+ TIME+  进程使用cpu的时间
+ COMMAND   进程名称![img](https://img-blog.csdnimg.cn/eb337fcbaa0346b88026f597964e1428.png)
...
+ ps![image-20230903193845417](/Users/z.m/Library/Application Support/typora-user-images/image-20230903193845417.png)
...
+ vmstat  ![img](https://img-blog.csdnimg.cn/a73bb50b0c234fd884ab690b16bfac9f.png)
  + 这个命令是系统自带
  + 虚拟内存统计的缩写，可对虚拟内存、进程、cpu活动进行监控![img](https://img-blog.csdnimg.cn/a87271c41aad4637ac5b9a9e6249b53b.png)
...
+ cpu
  ...
  + wa 
  + st
+ sysstat的工具包，这个工具包中带了很性能分析命令`yum install sysstat -y`![img](https://img-blog.csdnimg.cn/53745298d91f4fc9928fd6c209553adf.png)
  + mpstat  查看cpu的相关数据
  + mpstat -P ALL 2 5
    +  %usr   
    + %nice   
...
+ pidstat： 能看磁盘、内存、cpu的数据，主要看cpu的上下文数据![img](https://img-blog.csdnimg.cn/51cc3234a62c4135ad9a6d006410683f.png)
...
+ iostat：  看数据换入换出![img](https://img-blog.csdnimg.cn/318d4e1c03a9428bbfe1426eb3fcae2d.png)
...
![img](https://img-blog.csdnimg.cn/8af98868f3e5481d9fae4d18f2c1e203.png)
...
+ 性能分析命令：
  ...
  + netstat
  + dstat
  + sar：  主要用在网络相关数据的查看![img](https://img-blog.csdnimg.cn/05d2bac6ea974a9cb55e8c39ad11e94b.png)
    + -n  网络相关数据统计
    + -d -b  磁盘驱动
    + -r  内存相关
    + -u  -P  cpu相关![img](https://img-blog.csdnimg.cn/9efaaf45cd844a6fa2e1cb5493417d3f.png)

块：从磁盘中读取数据的 最小单位

页：从内存中交互数据的 最小单位   页的数据量大小要比 块要大
...
+ cpu  中央处理器
  + 架构、主频、核
+ cpu：结构 主要物理结构是3个，实际是有4![img](https://img-blog.csdnimg.cn/6da75b2c4a6d415a83c2c01387415ba3.png)![img](https://img-blog.csdnimg.cn/3b361f9ca0c14d69b50abb9d8dcafa22.png)![img](https://img-blog.csdnimg.cn/43a847a8f748419094a4e75e95f1e8aa.png)
  + 运算器： 真正进行计算的单元
  + 控制器： leader
  + 寄存器： 存储 指令、数据、地址
  + 时钟
...
cpu中： 内核、线程、架构![img](https://img-blog.csdnimg.cn/21f4b34e7a754e0dbadc3a08deca1cc9.png)

cpu的内核，医院中的医生

cpu的线程，医生配备的护士

cpu的架构，医院的结构![img](https://img-blog.csdnimg.cn/7d3b50ae976b46be922028e42ed7e11b.png)

看cpu的数据信息： `top`  `lscpu`  `cat /proc/cpuinfo`![img](https://img-blog.csdnimg.cn/d05b9bc4ad4b40b5a9f3bf868eecd104.png)
...
![img](https://img-blog.csdnimg.cn/9e2131caf76d425d9768e8804c7c6796.png)

## 上下文

![img](https://img-blog.csdnimg.cn/3ffe231efc8c469999ae84fb61ccbc16.png)

![img](https://img-blog.csdnimg.cn/5969b5d5512c4b1b9f5c781304c41dc5.png)

![img](https://img-blog.csdnimg.cn/a80e3c7f29fe4917a281d6201aba5740.png)

![img](https://img-blog.csdnimg.cn/a34e29e8cc5b40c4bb38dd28f2c265ed.png)
...
+ 服务器硬件资源监控： grafana(前端) + prometheus(时序数据库) + node_exporter(硬件资源收集器)![img](https://img-blog.csdnimg.cn/0347bf0d68cb43198c6760be221a6a90.png)
...
+ stress-ng:  性能测试模拟工具，可以直接模拟服务器各种压力情况
  ...
  + 安装![img](https://img-blog.csdnimg.cn/22615c5ba8504dca843d377eb1c293ef.png)
...
总结：![img](https://img-blog.csdnimg.cn/607ab7b94a28422a9e1777202884d811.png)![img](https://img-blog.csdnimg.cn/a63f2fd5cf91467f9e14857b4df8cb4f.png)
...
+ 结论![img](https://img-blog.csdnimg.cn/3d68a7196f77464aa3ec45c5f3a839e7.png)
...
![image-20230905215736625](/Users/z.m/Library/Application Support/typora-user-images/image-20230905215736625.png)
...
![img](https://img-blog.csdnimg.cn/551295eb81344c51b719bc4feb7fc401.png)
...
===你们考虑一下，这个问题这么解决？
	...
	----已经定位到了具体是哪个程序导致了
		...
		比如日志级别打得比较低，会持续进行读写操作，会产生大量的IO
...
+ 内存知识![img](https://img-blog.csdnimg.cn/eb8f8627544942bf9512b0e842becdb6.png)![img](https://img-blog.csdnimg.cn/78a37705776a4c8e8a248c7d384ed52a.png)
...
+ 内存中，内存地址与存储单元组成的.  数据结构![img](https://img-blog.csdnimg.cn/c7a57ce7ab4f46e3be6a86350d805757.png)
  + 存储单元中，就是来真正存储内容
  + 不同的数据类型，存储单元大小不一样
    + int float, char
    + 列表  数组
    + python：列表 [8,'nmb',['vip8','vip12'],]
      ...
      + 所以，这种，速度是比较慢
+ 数据结构
  + 链表：  自行车链条![img](https://img-blog.csdnimg.cn/63103e937be74e0595d2849a61234d89.png)
...
+ 堆栈：  两种数据结构![img](https://img-blog.csdnimg.cn/5d068d6ea0044d7e80b4cf5bc52bdb21.png)
  + 栈： LIFO
  + 队列：FIFO![img](https://img-blog.csdnimg.cn/014da735889f448e89aae78e1df79eb8.png)
  + 堆：![img](https://img-blog.csdnimg.cn/14f970b1ca584c4db218ef07f2ac6ef0.png)

![img](https://img-blog.csdnimg.cn/56e424f8d17846e69f37d12bfab20065.png)

​	![img](https://img-blog.csdnimg.cn/d20ce0f07efd4dd1a61a22ba91ab1c79.png)
